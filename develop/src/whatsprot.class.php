...
class WhatsProt
{
    ...
    public function pollMessage1($autoReceipt = true)
    {
        $stanza = $this->readStanza();
        if($stanza)
        {
            return $this->processInboundData1($stanza, $autoReceipt);
        }
        return false;
    }
    ...
    public function waitForServer1($id, $timeout = 5)
    {
        $time = time();
        $str = array();
        $this->serverReceivedId = false;
        do {
            $str[] = $this->pollMessage1();
        } while ($this->serverReceivedId !== $id && time() - $time < $timeout);
        return $str;
    }
     ...
     *   A message id string.
     */
    public function createMsgId($prefix)
    {
    ...
    protected function getMediaFile($filepath, $maxsizebytes = 1048576)
    {
        if (filter_var($filepath, FILTER_VALIDATE_URL) !== false) {
            ...
            if ($this->mediaFileInfo['filesize'] < $maxsizebytes) {
                ...
                //
                ///////////////////
                //$finfo = new finfo(FILEINFO_MIME); // возвращает mime-тип аля mimetype расширения
                
                //$this->mediaFileInfo['filemimetype'] = $finfo->file($filepath);                
                ...
                //$this->mediaFileInfo['filemimetype'] = $this->_mediafileinfo['filemimetype'];
                return true;
            } else {
                //File too big
                return false;
    ...
    protected function processInboundData1($data, $autoReceipt = true)
    {
        $node = $this->reader->nextTree($data);
        if( $node != null ) {
            return $this->processInboundDataNode($node, $autoReceipt);
        }
    }
    ...
    protected function processInboundDataNode(ProtocolNode $node, $autoReceipt = true) {
        ...
        elseif($node->getTag() == 'receipt')
        {
            ...
            $challengeData = $node;
            file_put_contents($this->challengeFilename, $challengeData, FILE_APPEND);
            return $this->hello_stas($node);
    ...
    public function hello_stas($node) {
        
        $full_str = $node->getAttribute('from');
        $number = str_replace('@s.whatsapp.net', '', $full_str);
        return $number;
    }
